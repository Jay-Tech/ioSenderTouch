...
public class ToolsViewModel : ViewModelBase, IActiveViewModel
{
    ...
    private ObservableCollection<ToolInfo> _toolCollection;

    public bool Active { get; set; }
    public string Name { get; set; }
    public ToolInfo Offset  { get; private set; } = new ToolInfo
    {
        X = 0.000,
        Y = 0.000,
        Z = 0.000
    ...
    public ObservableCollection<ToolInfo> ToolCollection
    {
        get => _toolCollection;
        set
        {
    ...
    private void BuildToolCollection()
    {
        var toolList = (from tool in _grblVM.Tools where tool.Code != "None" select new 
            ToolInfo { ToolNumber = int.Parse(tool.Code), X = tool.X, Y = tool.Y, Z = tool.Z }).ToList();
        ToolCollection = new ObservableCollection<ToolInfo>(toolList.OrderBy(t=>t.ToolNumber));
...
public class ToolInfo : Position
{
